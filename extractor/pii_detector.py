...
def style_for_span(line: Dict[str, Any], start: int, end: int) -> Tuple[Optional[str], Optional[float]]:
    ...
    if not best:
        return None, None
    return best.get("font"), best.get("size")
    ...
    for line in lines:
        ...
        for rule in rules:
            for match in rule.pattern.finditer(line_text):
                ...
                font, size = style_for_span(line, start, end)
...
def detect_with_spacy(page_no: int, lines: List[Dict[str, Any]], nlp: Any) -> List[Detection]:
    ...
    for line in lines:
        ...
        for ent in doc.ents:
            ...
            font, size = style_for_span(line, int(ent.start_char), int(ent.end_char))
            detections.append(make_detection(
                page_no=page_no,
                pii_type=pii_type,
                original_text=value,
...
def deduplicate(detections: List[Detection]) -> List[Detection]:
    ...
    sorted_items = sorted(
        detections,
        key=lambda d: (d.get("page_no", 0), d.get("type", ""), -float(d.get("confidence", 0))),
    )
    ...
    for item in sorted_items:
        ...
        for idx, existing in enumerate(kept):
            ...
            same_page = item.get("page_no") == existing.get("page_no")
    ...
    kept.sort(key=lambda d: (d.get("page_no", 0), (d.get("bbox") or [0, 0, 0, 0])[1], (d.get("bbox") or [0, 0, 0, 0])[0]))
    return kept
